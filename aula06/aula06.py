...
def convolute_image(image, mask: list[list[int]], operation: Callable):
    black_white_image = enhance_image(image, r1=154, r2=155)  # so tem 0 ou 255
...
def main():
    image = cv2.imread("arroz_feijao.jpg")
    convolute_image(image, [[1, 2, 3]], dummy)
...
def dummy(mask: list[list[int]], image_patch: list[list[int]]) -> bool:
